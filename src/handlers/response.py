...
class ResponseHandler:
        ...
        for tool_call in tool_calls:
            ...
            tool_entry: Dict[str, Any] = {
                "function": {
                    ...
                    "arguments": self._parse_tool_arguments(function_info.get("arguments"))
                }
            }
    ...
    def _merge_stream_tool_calls(
        ...
        for index, tool_call in enumerate(delta_tool_calls):
            ...
            state = buffers.setdefault(index, {"name": "", "arguments": "", "description": ""})
            function_info = tool_call.get("function", {})
            if not isinstance(function_info, dict):
                function_info = {}
    ...
    def _build_stream_tool_calls(self, buffers: Dict[int, Dict[str, str]]) -> List[Dict[str, Any]]:
    ...
    def _extract_chunk_content(self, chunk: Dict[str, Any]) -> Tuple[str, List[Dict[str, Any]], Optional[str]]:
        """
    ...
    def _create_tool_call_chunk(self, model: str, tool_calls: List[Dict[str, Any]]) -> str:
    ...
    def handle_streaming_response(
        ...
        # 스트림 시작 로그
        logger.info(f"[Stream] 📤 시작 | model={requested_model} | max_tokens={max_tokens}")
        ...
        try:
            for line in resp.iter_lines():
                ...
                text_content, delta_tool_calls, finish_reason = self._extract_chunk_content(parsed)
                self._merge_stream_tool_calls(pending_tool_call_buffers, delta_tool_calls)
                ...
                if finish_reason in ("stop", "tool_calls", "length"):
                    ...
                    yield self._create_final_chunk(requested_model, start_time, finish_reason)
                    stream_finished = True
                    break
    ...
    def handle_non_streaming_response(
        ...
        try:
            ...
            message: Dict[str, Any] = {"role": "assistant", "content": text_content.strip()}
            if tool_calls:
