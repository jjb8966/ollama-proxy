...
def list_models():
    ...
    try:
        ...
        return Response(
            json.dumps(empty_response), status=200, mimetype="application/json"
        )
    ...
    for m in models:
        ...
        if not model_id:
            continue
        result["data"].append(
            {"id": model_id, "object": "model", "created": 0, "owned_by": "proxy"}
        )
...
def chat_completions():
    ...
    for param in [
        ...
        if param in req:
            proxied_req[param] = req[param]

    # API 요청 처리
    resp = chat_handler.handle_chat_request(proxied_req)
    if resp is None:
        error_body = {"error": {"message": "API request failed", "type": "api_error"}}
